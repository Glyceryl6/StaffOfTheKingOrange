...
public class BoneMeal extends AbstractInvisibleProjectile {
    ...
    public static boolean applyBoneMeal(Level level, BlockPos pos) {
        ...
        if (blockState.getBlock() instanceof BonemealableBlock block && block.isValidBonemealTarget(level, pos, blockState, level.isClientSide)) {
                    ...
                    if (holder.is(BiomeTags.PRODUCES_CORALS_FROM_BONEMEAL)) {
                        if (i == 0 && clickedSide != null && clickedSide.getAxis().isHorizontal()) {
                            blockState = BuiltInRegistries.BLOCK.getTag(BlockTags.WALL_CORALS)
                                    .flatMap(blockHolder -> blockHolder.getRandomElement(level.random))
